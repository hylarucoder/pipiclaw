...
import { renderCodeBlock, renderTextDiff } from "./ArtifactRenderers.js";
	...
	render(
		...
		if (result?.isError) {
			...
			if (command === "create" || command === "update" || command === "rewrite") {
					...
					const diffContent =
						old_str !== undefined && new_str !== undefined ? renderTextDiff(old_str, new_str) : "";
				...
				return {
					...
					<div>
							...
							<div ${ref(contentRef)} class="max-h-0 overflow-hidden transition-all duration-300 space-y-3">
								${isDiff ? diffContent : content ? renderCodeBlock(content, getLanguageFromFilename(filename)) : ""}
								${
									isHtml
				...
				return {
					...
					<div>
							...
							<div ${ref(contentRef)} class="max-h-0 overflow-hidden transition-all duration-300">
								${renderCodeBlock(fileContent, getLanguageFromFilename(filename))}
							</div>
						</div>
					`,
					isCustom: false,
			...
			if (command === "create" || command === "rewrite") {
				...
				return {
					...
					<div>
							${renderCollapsibleHeader(state, FileCode2, renderHeaderWithPill(headerText, filename), contentRef, chevronRef, false)}
							<div ${ref(contentRef)} class="max-h-0 overflow-hidden transition-all duration-300 space-y-3">
								${codeContent ? renderCodeBlock(codeContent, getLanguageFromFilename(filename)) : ""}
			...
			if (command === "update") {
				...
				return {
					...
					<div>
							${renderCollapsibleHeader(state, FileCode2, renderHeaderWithPill(headerText, filename), contentRef, chevronRef, false)}
							<div ${ref(contentRef)} class="max-h-0 overflow-hidden transition-all duration-300 space-y-3">
								${renderTextDiff(params.old_str || "", params.new_str || "")}
								${isHtml && logs ? html`<console-block .content=${logs}></console-block>` : ""}
							</div>
						</div>
				`,
				...
				case "rewrite":
					return {
						...
						<div>
								...
								<div ${ref(contentRef)} class="max-h-0 overflow-hidden transition-all duration-300">
									${
										content ? renderCodeBlock(content, getLanguageFromFilename(filename)) : ""
									}
								</div>
							</div>
					`,
