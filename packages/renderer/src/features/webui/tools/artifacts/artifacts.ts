import type { Agent, AgentMessage, AgentTool } from "@pipiclaw/agent-core";
import type { ToolCall } from "@mariozechner/pi-ai";
import { StringEnum } from "@renderer/lib/piAiBrowserShim";
...
export class ArtifactsPanel extends HTMLElement {
	...
	private _sandboxUrlProvider?: () => string;
	// Callbacks
	private _onArtifactsChange?: () => void;
	...
	set agent(value: Agent | undefined) {
		...
		this._agent = value;
		this.refreshHtmlArtifactBindings();
		this.requestUpdate();
	}

	get sandboxUrlProvider(): (() => string) | undefined {
		return this._sandboxUrlProvider;
	}

	set sandboxUrlProvider(value: (() => string) | undefined) {
		if (this._sandboxUrlProvider === value) return;
		this._sandboxUrlProvider = value;
		this.refreshHtmlArtifactBindings();
		this.requestUpdate();
	}

	get onArtifactsChange(): (() => void) | undefined {
		return this._onArtifactsChange;
	}

	set onArtifactsChange(value: (() => void) | undefined) {
		this._onArtifactsChange = value;
	}
	...
	private refreshHtmlArtifactBindings() {
							...
							${artifacts.map((a) => {
								...
								return html`
									<button
										...
										class="inline-flex h-9 items-center rounded-none border-b-2 px-3 py-2 font-mono text-xs whitespace-nowrap transition-colors hover:bg-muted/40 ${activeClass}"
