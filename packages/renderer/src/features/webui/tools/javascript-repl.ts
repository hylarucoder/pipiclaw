							...
							attachments.length
								? html`<div class="mt-3 flex flex-wrap gap-2">
									${attachments.map(
										(att) => html`<button
											type="button"
											class="inline-flex max-w-[220px] cursor-pointer items-center rounded-md border border-border bg-card px-2 py-1 text-xs text-foreground transition-colors hover:bg-muted"
											@click=${() => AttachmentOverlay.open(att)}
											title=${att.fileName}
										>
