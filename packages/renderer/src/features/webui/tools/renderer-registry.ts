...
export function renderHeader(
	...
	const statusIcon = (iconComponent: any, color: string) =>
		html`<span class="inline-block ${color}">${renderIcon(iconComponent, "sm")}</span>`;
...
export function renderCollapsibleHeader(
	...
	return html`
		<button
			type="button"
			@click=${toggleContent}
			class="flex w-full items-center justify-between gap-2 rounded-md px-0 py-1 text-sm text-muted-foreground transition-colors hover:text-foreground"
		>
			<div class="flex items-center gap-2">
			...
			<span class="inline-block text-muted-foreground" ${ref(chevronRef)}>
				<span class="chevron-up ${defaultExpanded ? "" : "hidden"}">${renderIcon(ChevronUp, "sm")}</span>
				<span class="chevrons-up-down ${defaultExpanded ? "hidden" : ""}">${renderIcon(ChevronsUpDown, "sm")}</span>
			</span>
		</button>
	`;
}
