...
type TranslationDictionary = Record<string, i18nMessages>;

let translationStore: TranslationDictionary = translations;

export function setTranslations(customTranslations: TranslationDictionary): void {
	translationStore = customTranslations;
}
...
export function getCurrentLanguage(): LanguageCode {
	...
	if (stored && translationStore[stored]) {
		return stored;
	}
	const userLocale = navigator.language || "en";
	const languageCode = userLocale.split("-")[0] || "en";
	return translationStore[languageCode] ? languageCode : "en";
}
...
export function i18n(category: string, key: string): string;
export function i18n(categoryOrKey: string, key?: string): string {
	const languageCode = getCurrentLanguage();
	const implementation = translationStore[languageCode] || translationStore.en;
